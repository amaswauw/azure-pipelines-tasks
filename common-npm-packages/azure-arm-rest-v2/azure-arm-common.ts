...
export class ApplicationTokenCredentials {
    ...
    public getToken(force?: boolean): Q.Promise<string> {
        ...
        if (!this.token_deferred || force) {
            ...
            {
                this.token_deferred = ApplicationTokenCredentials.getMSIAuthorizationToken(0, 0, this.msiClientId, this.baseUrl);
            }
            else
            {
            ...
            (response: webClient.WebResponse) => {
                ...
                else if (response.statusCode == 429 || response.statusCode == 500)
                {
                    if(retyCount < retryLimit)
                    {
                        ...
                        retyCount +=1;
                        setTimeout(() => {
                            deferred.resolve(this.getMSIAuthorizationToken(retyCount, waitedTime, msiClientId, baseUrl));   
                        }, waitedTime);
                    } 
                    else
                    {
